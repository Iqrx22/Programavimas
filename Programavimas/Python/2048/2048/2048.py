...
GRID_IMAGE = pygame.transform.scale(GRID_IMAGE, GRID_SIZE)
...
def isvesti_busena(lentele, didziausias):
    ...
    if didziausias <= 2048:
        ...
        WIN.blit(text_surface, (400, -10))
        for i in range(BOARD_SIZE):
            for j in range(BOARD_SIZE):
                what_points = status_font.render(str(lentele[i][j]), False, WHITE)
                WIN.blit(text_surface, (30+PIXELSPERBOX*i, 60+PIXELSPERBOX*j))
    else:
        text_surface = status_font.render('You lost!', False, WHITE)
