...
struct zodis_numeris generate_random_zodi(int kiek_raidziu) {
    ...
    cout<<"Ar galima naudoti lietuviškas raides? (T / N): ";
    char galima;
    cin>>galima;
        for (;;) {
...
void isvesti_busena(char rodykle[][50][2], int ilgis, int kuris_zodis, struct zodis_numeris a, bool rodyti_atsakyma) {

    system("CLS");
    if (rodyti_atsakyma) {
    cout<<"Žodžio numeris: "<<a.numeris<<". Atsakymas: "<<a.zodis<<"."<<endl;
    }

    cout<<"Žodis sudarytas iš "<<ilgis<<" raidžių"<<endl<<endl;
...
void ivestis(char rodykle[][50][2], int ilgis, int kuris_zodis, struct zodis_numeris a, bool rodyti_atsakyma) {
    ...
    if (spejimas.length() != ilgis){
        for (;spejimas.length() != ilgis;) {
           cout<<"Spėjimas yra neteisingo ilgio. Įveskite spėjimą iš "<<ilgis<<" raidžių: ";
           cin>>spejimas;
        }
    }
    for (int i = 0; i < ilgis; i++) {
...
int main() {
    ...
    cout<<"Kiek norite turėti raidžių žodyje? Įveskite \"0\", kad būtų parinktas atsitiktinis raidžių skaičius: ";
    ...
    cout<<"Kiek norite turėti spėjimų? Įveskite \"0\", kad gautute vienu daugiau spėjimų nei raidžių žodyje: ";
    ...
    /*char raides[2][32] = {
        {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'y', 'j', 'k', 'l', 'm', 'n', 'o' , 'p', 'r', 's', 't', 'u', 'v', 'z', 'à', 'è', 'æ', 'ë', 'á', 'ð', 'ø', 'û', 'þ'},
        {'-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-'}
    } */
    ...
    cout<<"Įveskite slaptažodį, kad būtų parodytas atsakymas, arba \"N\", kad nebūtų: ";
    string slaptazodis;
    cin>>slaptazodis;
    ...
    for (int i = 0; i < spejimu_skaicius; i++) {
        ivestis(lentele, ilgis, i, a, rodyti_atsakyma);
        isvesti_busena(lentele, ilgis, i, a, rodyti_atsakyma);
        ...
        if (ar_zodis_teisingas == false) {
            cout<<"Žodis dar neatspėtas."<<endl;
        }
        else {
            cout<<"Jūs atspėjote žodį! Jis buvo "<<secret_zodis;
            return 0;
        }
    }
    cout<<"Jūs neatspėjote žodžio. Teisingas atsakymas buvo "<<secret_zodis;
